<?php
  ...
  $cid = isset($_POST['class_id']) ? $_POST['class_id'] : null;
  $number = isset($_POST['number']) ? $_POST['number'] : null;
  $mode = isset($_POST['mode']) ? $_POST['mode'] : null; // 0 英文 1 中文 2 混搭
  ...
  if ($cid && $number && isset($mode)) {
    ...
    if (count($_empty) !== 0) {
      ...
      $word_sql = ($cid / 1 === 1) ? "SELECT `id`, `word`, `explanation` FROM `word`" : "SELECT `id`, `word`, `explanation` FROM `word` WHERE `type_id` = '$cid'";
